...
class RockTaskController < ApplicationController
  ...
  def rock_task_params
    params.require(:rock_task).permit(:name, :rock_task_grade_id, :youtube_url, :other, images: []).merge(
      user_id: current_user.id, rock_information_id: params[:rock_information_id]
    )
  end
end
